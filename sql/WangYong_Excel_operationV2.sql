...
INSERT INTO excel_parts ( `excel_id`, `sheet_sql`, `head_sql`, `data_sql`, `template`, `sort`, `state`) VALUES ( @excelId, 'select DISTINCT date_format(t.occurdtime ,\'%Y-%m-%d\') id,date_format(t.occurdtime ,\'%Y-%m-%d\') name from sites s left join tank_operation t on t.siteid = s.id where t.state = 1 ', NULL, 'select t.id,t.siteid,t.faulttype,t.subtype,t.subtypename,t.desc,t.procedure,t.changedev,t.createtime,t.creator,t.company,t.state,\r\n(select group_concat(name) from dict where group_code = \'OPERATION.DEVICE\' and find_in_set(code, t.changedev)) changedevname, \r\nt.occurdtime ,t.solvetime, \r\n(case t.faulttype when \'1\' then \'离线\' else \'告警\' end) faulttypename,s.name from tank_operation t \r\nleft join sites s on t.siteid = s.id \r\nwhere t.state = 1 and date_format(t.occurdtime ,\'%Y-%m-%d\') = \'#id\' #filter\r\norder by t.id asc ', @templateTemp, '1', '1');
